...
@borg.on(admin_cmd("PrimeUserbot ?(.*)"))
async def _(event):
    if event.fwd_from:
        return
    ...
    if sysarg == "hello":
      async with borg.conversation(bot) as conv:
          try:
              ...
              await borg.send_file(event.chat_id, audio, caption="⚡Boss Listen To This Audio" + naam +"\n`Check out` [PrimeUserbot](https://github.com/PrimeUserbot/PrimeUserbot)")
              await event.delete()
          except YouBlockedUserError:
              await event.edit("**Error:** `unblock` @PrimeUserbot `and retry!")
    elif sysarg == "help":
      async with borg.conversation(bot) as conv:
          try:
              await conv.send_message("/start")
      ...
      async with borg.conversation(bot) as conv:
          try:
              ...
              await event.edit("**Error:** `unblock` @PrimeUserobot `and retry!`")
    elif sysarg == "song":
      async with borg.conversation(bot) as conv:
          try:
              await conv.send_message("/start")
